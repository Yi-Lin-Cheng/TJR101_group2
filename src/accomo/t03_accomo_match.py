...
def main():
    ...
    for _, row in df.iterrows():
        ...
        if (
            (
                name_open == name
                or name_open_clean == name_clean
                or name_clean in name_open_clean
            )
            and add_open_clean == add_clean
    ...
    def compute_distance(row):
        if (
            pd.notna(row["lat_open"])
            and pd.notna(row["lat"])
            and pd.notna(row["lng_open"])
    ...
    # -------- 挑選比對佳者 --------
    df = df.sort_values(
        by=["name", "lat", "lng", "match_score", "distance_km"],
    ...
    # -------- 新增 rating_score5 欄位 --------
    df.insert(
        df.columns.get_loc("rating") + 1,
        "rating_score5",
        df["rating"].apply(lambda x: round(x / 2, 1) if pd.notna(x) else x),
    )
    ...
    # -------- 輸出檔案 --------
    output_path = data_dir / "accomo05_match_result.csv"
